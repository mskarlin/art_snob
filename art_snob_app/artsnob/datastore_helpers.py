...
import os
from artsnob.ordered_set import OrderedSet
import itertools
import sys
...
class FriendlyDataStore():
    ...
    def similar_art(self, id_list, hydrated=False, interleaved_results=True):
        ...
        if hydrated:
            unique_art = self.dsi.read(unique_art, kind=self.INFO_KIND, sorted_list=True)
    ...
    def random(self, kind=None, seed=814, n_items=1):
        ...
        keys = self.dsi.read(rand, self.RANDOM_INDEX_KIND)[:n_items]

        return self.dsi.read(keys, kind, sorted_list=True)
